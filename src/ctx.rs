...
impl<F: ObjFunc> Ctx<F> {
    pub(crate) fn from_parts(
        ...
        pool: Vec<Vec<f64>>,
        mut pool_f: Vec<F::Fitness>,
    ) -> Self {
        best.update_all(&pool, &pool_f);
        pool_f.iter_mut().for_each(|f| f.mark_not_best());
        Self { best, pool, pool_f, gen: 0, func }
    }
    ...
    pub(crate) fn from_pool(func: F, best: BestCon<F::Fitness>, pool: Vec<Vec<f64>>) -> Self {
        ...
        Self::from_parts(func, best, pool, pool_f)
    }
    ...
    pub fn find_best(&mut self) {
        self.best.update_all(&self.pool, &self.pool_f);
        self.prune_fitness();
    }
}
